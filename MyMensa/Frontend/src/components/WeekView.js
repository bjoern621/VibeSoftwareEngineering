...
function WeekView() {
  ...
  useEffect(() => {
    ...
    loadMealPlan();
  }, [weekOffset]); // Wird neu geladen, wenn weekOffset sich ändert
  ...
  const getWeekStart = (offset) => {
    ...
    const diff = dayOfWeek === 0 ? -6 : 1 - dayOfWeek; // Berechne Differenz zum Montag

    const monday = new Date(today);
    monday.setDate(today.getDate() + diff + (offset * 7)); // Füge Wochenoffset hinzu
    monday.setHours(0, 0, 0, 0); // Setze Zeit auf Mitternacht
  ...
  /**
   * Formatiert ein Datum schön (z.B. "13.10.2025")
   */
